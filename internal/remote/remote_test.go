...
func TestDockerRestartCommand(t *testing.T) {
	...
	want := []string{"-p", "22", "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes", "root@10.0.0.1", "docker restart 'frank'"}
...
func TestScriptCommandAppendsOperatorUser(t *testing.T) {
	...
	want := []string{"-p", "22", "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes", "root@10.0.0.1", "/prosh/frankzhou/unzip_re_docker_test.sh -q --user 'Frank Zhou'"}
	if !equal(cmd.Args, want) {
		t.Fatalf("args mismatch\nwant %#v\n got %#v", want, cmd.Args)
	}
}
...
func TestTailCommandPrintsRequestedLines(t *testing.T) {
	...
	want := []string{"-p", "22", "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes", "root@10.0.0.1", "tail -n 50 '/data/logs/catalina.out'"}
	if !equal(cmd.Args, want) {
		t.Fatalf("args mismatch\nwant %#v\n got %#v", want, cmd.Args)
	}
}
...
func TestTailCommandUsesDefaultTailLines(t *testing.T) {
	...
	want := []string{"-p", "22", "-o", "StrictHostKeyChecking=no", "-o", "BatchMode=yes", "root@10.0.0.1", fmt.Sprintf("tail -n %d '/data/logs/catalina.out'", constants.DefaultTailLines)}
	if !equal(cmd.Args, want) {
		t.Fatalf("args mismatch\nwant %#v\n got %#v", want, cmd.Args)
	}
}
