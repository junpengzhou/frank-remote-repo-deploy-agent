...
func TestBuildCommandAddsSSHConnectTimeoutOnly(t *testing.T) {
	cmd := BuildCommand("/tmp/staging/frank", "/data/frank", Options{
		ConnectTimeoutSeconds: 10,
		SSH:                   config.SSHConfig{User: "root", Host: "10.0.0.1", Port: 2222},
	})
	want := []string{"-az", "--delete", "--partial", "-e", "ssh -p 2222 -o StrictHostKeyChecking=no -o BatchMode=yes -o ConnectTimeout=10", "/tmp/staging/frank/", "root@10.0.0.1:/data/frank"}
	if !equal(cmd.Args, want) {
		t.Fatalf("args mismatch\nwant %#v\n got %#v", want, cmd.Args)
	}
}
